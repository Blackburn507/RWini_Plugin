...
function activate(context) {
	...
	function getAllCustomVariableCompletion(document){
		let customVariableCompletions=
			new getCustomVariable(document).memoryNumber.map(names=>{return new vscode.CompletionItem('memory.'.concat(names),vscode.CompletionItemKind.Constant)}).concat(
			new getCustomVariable(document).memoryFloat.map(names=>{return new vscode.CompletionItem('memory.'.concat(names),vscode.CompletionItemKind.Constant)})).concat(
			new getCustomVariable(document).memoryBool.map(names=>{return new vscode.CompletionItem('memory.'.concat(names),vscode.CompletionItemKind.Constant)})).concat(
    ...
    const logicValueProvider = vscode.languages.registerCompletionItemProvider('rwini',{
	    provideCompletionItems(document,position){
			...
			if (['logicBoolean','dynamicFloat','unitRef'].includes(allKeys[currentSection][currentKey[0]][0])){
	...
	const fileJumpHoverPrompt = vscode.languages.registerHoverProvider('rwini', {
        provideHover(document, position){
            let range = document.getWordRangeAtPosition(position, /(ROOT:)?[\w\.\/-]+/i);
			...
			if (currentKey!==null && currentKey[0].match(/(body|leg|arm|effect|mutator)\d?_/)==null){
				...
				if (allKeys[currentSection]!=undefined && currentSection!='' && getIfMultiline(event.document,keyStart)==false ){
					...
					//Check value
						...
						const preDiagnostic = new vscode.Diagnostic(range,`Unexpected value:\'${currentValue}\'(type=\'${valueType[currentValue]}\'), Key: ${currentKey}(expectedType=\'${expectedValue}\')`,vscode.DiagnosticSeverity.Error);
