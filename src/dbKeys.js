const allKeys = {
  core: {
    ...
    buildSpeed: [
      ...
      'core',
      '建造速度',
      '建造此单位需要的时间,填秒.以前的计算方式为:此处所填时间=1÷(60x你需要的秒)如果定义了工厂速率则需要乘以建造乘数.',
    ],
    radius: [
      'int',
      'core',
    ...
    energyRegenWhenRecharging: [
      ...
      'core',
      '充能时能量恢复速度',
      '能量恢复是持续的,如果你设置了energyNeedsToRechargeToFull,那么攻击时按energyRegen恢复,耗尽时的灰条按此处设定值恢复.',
    ],
    ...
    showActionsWithMixedSelectionIfOtherUnitsHaveTag: [
      ...
      'core',
      '混合所选单位所显示的行为(标签)',
      '如果选择的单位都包含此处使用的标签,则合并“行为(action)”.比如你的步单位通过部署转化成另一个单位,在混合时可以当作同一个单位处理,不再是默认的谁都无法执行操作.例子如红警的盟军大单位在混合选中后依旧可执行部署或解除.',
    ],
    ...
    selfBuildRate: [
      ...
      'core',
      '自动建造速度',
      '此单位自动构造所需要的时间,目前用于虫族.计算方式为:此处所填时间=1÷(60x你需要的秒).',
    ],
    ...
    onNewMapSpawn: [
      ...
      'core',
      '地图位置产生指定单位',
      '在地图上添加此单位.可以用于覆盖铁锈默认的资源逻辑.参数:emptyResourcePools_asNeutral 产生位置于空资源池为中立所属, emptyOrOccupiedResourcePools_asNeutral空的或者已经占用的资源池为中立所属, mapCenter_asNeutral地图中心为中立所属, mapCenter_eachActiveTeam地图中心给与每个队伍, spawnPoint_eachActiveTeam玩家初始位置给与每个队伍',
    ],
    globalScale: [
      'float',
      'core',
    ...
    defineUnitMemory: [
      ...
      'core',
      '定义内存变量',
      '为单位创建一个自定义变量(作用域为单个ini). 支持的类型:boolean(bool),float,number,unit,string,text,boolean[],float[],number[],unit[]. (格式:defineUnitMemory:boolean nukeActive, float[] newHp)',
    ],
    updateUnitMemory: [
      'memory',
      'core',
      '更新单位内存',
      '类似[action]setUnitMemory但是性能更好',
    ],
    ...
    soundOnAttackOrder: [
      ...
      'core',
      '攻击指令音效',
      '填音效名称列表.用,隔开.每次下达攻击指令时只播放其中一个.仅支持.ogg和.wav格式,注意如果你mod用于pc端,则不要直接修改后缀名,要转换格式.',
    ],
    soundOnMoveOrder: [
      'file-sound',
      'core',
    ...
    nanoReclaimSpeed: [
      ...
      'core',
      '回收速度',
      '默认为nanoRepairSpeed*5.1',
    ],
    nanoRangeForRepairIsMelee: [
      'bool',
      'core',
      '维修范围按近战逻辑',
      '定义此单位维修时是否为近战算法,默认计算是从自身中心到目标中心,如果单位过大运动会受到阻碍,则虽然看起来很近然而实际中心太远无法修复. 近战类算法则是算上其半径,由中心计算改为从边缘计算.',
    ],
    ...
    builtFrom_x_name: [
      ...
      'core',
      '从此单位建造',
      '除非你向原版工厂添加单位,否则不建议使用.将该单位添加到目标建筑物中.',
    ],
    builtFrom_x_pos: [
      'float',
      'core',
    ...
    builtFrom_x_forceNano: [
      'bool',
      'core',
      '从此单位像建筑一样建造',
      '如果为true,则像构建建筑一样造此单位. (即使是一个单位)',
    ],
    builtFrom_x_isLocked: [
      'logicBoolean',
      'core',
      '从此单位建造条件',
      '如果为true,则无法在目标中构建此单位. (逻辑布尔值,设置建造条件)',
    ],
    builtFrom_x_isLockedMessage: [
      'string',
      'core',
      '从此单位建造锁定信息',
      '无法建造时显示的信息.',
    ...
    explodeTypeOnDeath: [
      ...
      'core',
      '死亡时爆炸类型',
      '可用的值:verysmall, small, normal, large, largeUnit, building, buildingNoShockwaveOrSmoke, verylargeBuilding',
    ],
    ...
    autoTriggerCheckRate: [
      ...
      'core',
      '自动触发检查率',
      '选项:everyFrame(默认), every4Frames, every8Frames. 注:所有触发器无论检查率都是在第一次创建和自动触发冷却之后检查的. 注:应用every8Frames到所有单位,可以让使用了复杂autoTriggers的mod有巨大的性能提升.',
    ],
    '@memory': [
      'other',
      'core',
      '定义内存变量',
      'defineUnitMemory的快捷方式(格式:@memory name:type)',
    ],
  },
  ...
  graphics: {
    ...
    image_offsetX: [
      ...
      'graphics',
      '图像X轴偏移',
      '图像在X轴偏移,用于不对称图像,比如你的单位有个吊臂,这样中心不在正中间.',
    ],
    image_offsetY: [
      'int',
      'graphics',
    ...
    animation_direction_starting: [
      ...
      'graphics',
      '多向动画朝向',
      '第一帧的方向,取决于你的素材.你要考虑转向后避免正负数交替现象,否则会导致素材朝向异常.',
    ],
    disableLowHpFire: [
      'bool',
      'graphics',
  ...
  attack: {
    ...
    turretMultiTargeting: [
      ...
      'attack',
      '每个炮塔向不同目标射击',
      '允许每个炮塔同时向不同的目标射击.如果使用turretlimitingAngle(限制角度)非常有用',
    ],
    isMelee: [
      'bool',
      'attack',
    ...
    stopTargetingAfterFiring: [
      ...
      'attack',
      '自动停火',
      '单位射击后停止瞄准.很少使用或需要.如用于你不希望胡乱攻击浪费弹药的单位.',
    ],
  ...
  turret: {
    ...
    canAttackMaxAngle: [
      ...
      'turret',
      '可攻击最大角度',
      '设置炮塔的最大射击角度.如果单位在可攻击角度外则不会开火.默认值为5,不要设置得更低.可以设置为181,即不需要转向就可发射炮抛射体.',
    ],
  ...
  projectile: {
    life: [
      ...
      'projectile',
      '存在时间',
      '开火后炮抛射体在没命中时的存活时间,单位为帧.60为1s.多少合适取决于速度与射程.',
    ],
    deflectionPower: [
      'float',
      'projectile',
    ...
    autoTargetingOnDeadTarget: [
      ...
      'projectile',
      '自动切换目标',
      '如果目标死亡则自动切换单位.填true时如果有代码"targetGround:true"炮抛射体将会追踪距离它最近的单位.如果没有"targetGround"时将会在单位死亡后再改变目标.',
    ],
    ...
    ballistic: [
      ...
      'projectile',
      '抛射体道',
      '抛射体道导抛射体效果,使抛射体先飞向空中并向下飞,而不是走直线.',
    ],
    ballistic_delaymove_height: [
      'float',
      'projectile',
      '抛射体道垂直飞行高度',
      '抛射体道导抛射体发射初期效果,决定抛射体道一开始先升高多少,再斜向爬升到最高过度.',
    ],
    ballistic_height: [
      'float',
      'projectile',
    ...
    instantReuseLast_keepAreaDamageList: [
      ...
      'projectile',
      '命中回收抛射体区域伤害列表',
      '瞬间命中并回收最后的抛射体并且保持区域伤害列表,默认false,保持列表是1.13的正常行为,造成区域伤害不会第二次生效,但不是很有用.只有当你想要旧的行为时才使用这个.',
    ],
    ...
    trueGravity: [
      ...
      'projectile',
      '绝对向重力',
      '炮弹在靠近单位时因为有该key会低空飞行一段距离,但注意不要填的太大,否则会直接往屏幕下方飞走.',
    ],
    turnSpeed: [
      'float',
      'projectile',
      '转弯速度',
      '限制抛射体的转弯速度,更好的做出抛射体道效果.',
    ],
    turnSpeedWhenNear: [
      'bool',
      'projectile',
  ...
  movement: {
    movementType: [
      ...
      'movement',
      '运动类型',
      '定义单位将能够通过的地形以及与单位类型相关的其他属性.类型:LAND,BUILDING,AIR,WATER,HOVER,OVER_CLIFF,OVER_CLIFF_WATER',
    ],
  ...
  ai: {
    ...
    useAsTransport: [
      ...
      'ai',
      '用作运输',
      '作为载具.如果单位可以运输单位,则默认为true.如果你的单位可以运输又能攻击,则AI可能囤积起来而不攻击,则需要设置false.',
    ],
    ...
    notPassivelyTargetedByOtherUnits: [
      ...
      'ai',
      '不被动地被其他单位瞄准',
      '允许更好的墙壁建筑物,默认情况下,这些建筑物不会瞄准目标.用于围墙',
    ],
  ...
  effect: {
    ...
    priority: [
      ...
      'effect',
      '优先级',
      '默认为高,一次显示太多效果时依据重要程度丢弃不重要的以用于创建新的.verylow/low/high/veryhigh/critical(很低,低,高,很高,至关重要)',
    ],
    attachedToUnit: [
      'bool',
      'effect',
    ...
    xOffsetRelative: [
      ...
      'effect',
      'X相对偏移量',
      '效果起始时相对于炮塔,炮抛射体,单位的偏移的方向位置.正数向右,负数向左.',
    ],
    yOffsetRelative: [
      'float',
      'effect',
      'Y相对偏移量',
      '效果起始时相对于炮塔,炮抛射体,单位的偏移的方向位置.正数向前,负数向后.',
    ],
    ...
    animateFrameStartRandomAdd: [
      ...
      'effect',
      '动画帧随机开始添加',
      '以动画起始帧为基准,按照+/-所填数字内随机选取数字进行帧随机偏移',
    ],
  },
  ...
  animation: {
    onActions: [
      ...
      'animation',
      '动画条件',
      '可用的动画触发的状态有:move, attack, idle, underConstruction, underConstructionWithLinkedBuiltTime, queuedUnits,Unknown, repair, reclaim, created',
    ],
  ...
  action: {
    ...
    displayType: [
      ...
      'action/hiddenAction',
      '显示类型',
      '可用的行动的显示类型有:none, rally, upgrade, queueUnit, building, action, infoOnly, infoOnlyNoBox, infoOnlyStockpile',
    ],
    ...
    guiBuildUnit: [
      ...
      'action/hiddenAction',
      '界面显示单位',
      '将action图标和鼠标指针改为指定单位,这就像你用建造者去造建筑,会受地形影响,可为action或路径点提供坐标.',
    ],
    tags: [
      'string',
      'action/hiddenAction',
      '标签',
      '可与queueSize(withActionTag=x)和queueItemAdded(withActionTag=x)结合使用',
    ],
    textAddUnitName: [
      'unitRef/unit',
      'action/hiddenAction',
    ...
    resetCustomTimer: [
      ...
      'action/hiddenAction',
      '重置自定义计时器',
      '重置自定义计时器,与self.customTimer()结合使用',
    ],
    setBodyRotation: [
      'dynamicFloat',
      'action/hiddenAction',
    ...
    setUnitStats: [
      ...
      'action/hiddenAction',
      '设置单位状态',
      '字段值.允许在不转换的情况下动态更改选定的字段数量.支持=/+=/-=,动态数学/逻辑.',
    ],
    resetUnitStats: [
      'bool',
      'action/hiddenAction',
    ...
    shrinkArrays: [
      ...
      'action/hiddenAction',
      '缩减数组',
      '在数组中清除一些不需要的值并向前收缩(unit[]中的死亡或是被删除的单位(null),float[]和number[]中的0)',
    ],
    setUnitMemory: [
      'memory',
      'action/hiddenAction',
      '设置单位内存',
      '键值对.改变此单元的内存,值可以用逻辑设定.内存必须首先用defineUnitMemory定义.',
    ],
    alsoTriggerAction: [
      'actionRef',
      'action/hiddenAction',
    ...
    fireTurretXAtGround_onlyOverPassableTileOf: [
      ...
      'action/hiddenAction',
      '指定攻击地面类型',
      '设置手动选择的地面需要满足这种运动方式.列表:NONE LAND BUILDING AIR WATER HOVER OVER_CLIFF OVER_CLIFF_WATER',
    ],
    spawnUnits: [
      'unitRef',
      'action/hiddenAction',
    ...
    addWaypoint_target_nearestUnit_team: [
      ...
      'action/hiddenAction',
      '添加路径点靠近队伍',
      '添加的路径点目标需要的所属方:own|notOwn|neutral|allyNotOwn|ally|enemy|any',
    ],
    ...
    takeResources_includeUnitsWithinRange_team: [
      ...
      'action/hiddenAction',
      '提取资源队伍',
      '提取在此范围内队伍的资源.与“includeUnitsWithinRange”一起使用,默认为own.可以是:own|notOwn|neutral|allyNotOwn|ally|enemy|any',
    ],
    ...
    takeResources_searchOnly: [
      ...
      'action/hiddenAction',
      '获取资源仅供搜索',
      '提取资源完全用于搜索范围内的单位而不是改动资源,与forEach配合使用',
    ],
    ...
    transportTargetNow: [
      ...
      'action/hiddenAction',
      '主动装运目标',
      '立即主动装载目标(1.15版本无法正常使用,会导致单位装载自己)',
    ],
    id: [
      'int',
      'action/hiddenAction',
    ...
    takeResources_triggerActionForEach: [
      'actionRef',
      'action/hiddenAction',
      '为提取资源找到的每个单位(提取目标)调用这个动作',
      '遍历提取资源搜索到的每个单位,将thisActionTarget设为该单位并触发填入的action',
    ],
    aiTags: [
      'string',
      'action/hiddenAction',
    ...
    aiUse: [
      ...
      'action/hiddenAction',
      'AI对该行动的使用方式',
      '可填:auto(自动),launch(发射),launchAmmo(增加弹药),disabled(残缺),upGrade(升级),movementChange(运动改变),sameAsBuilding(与建筑物相同)',
    ],
    alsoTriggerActionRepeat: [
      'dynamicFloat',
      'action/hiddenAction',
      '重复触发行动次数',
      '每次重复时自动index++, 用于创建循环或处理数组.',
    ],
    autoTriggerCheckRate: [
      'other',
      'action/hiddenAction',
  ...
  decal: {
    layer: [
      ...
      'decal',
      '绘制图层,可以是前面几种',
      '可以填入的值:shadow(阴影), beforeBody(在主体之上), afterBody(在主体之下), onTop(在最顶端), beforeUI(在显示界面之上),inactive(在行动中)',
    ],
    order: [
      'float',
      'decal',
      '排序(默认为0并且按照ini里的顺序)',
      '使用其他贴花排序,默认为0,并使用其在ini中显示的顺序.',
    ],
    ...
    onlyTeam: [
      ...
      'decal',
      '仅在查看者为特定队伍时显示',
      '仅当单位和查看玩家之间的关系时绘制,可以填写:己方|非己方|中立方|除自己外盟友|盟友|敌方|任意',
    ],
    ...
    onlyWhileAlive: [
      ...
      'decal',
      '仅在存活时绘制',
      '如果图层(layer)为beforeUI默认为true,否则false,为ture死后消失',
    ],
    onlyInPreview: [
      'bool',
      'decal',
    ...
    imageStack: [
      ...
      'decal',
      '图像列表',
      '推荐用于三维立体体积元素样式单位的图像列表,因为当使用相同的子画面时,可以批量绘制图像.)(目前没有实例所以不清楚,但确定是用于3d单位',
    ],
    stack_hOffset: [
      'int',
      'decal',
    ...
    stack_indexCount: [
      ...
      'decal',
      '列表图像数量',
      '动态整数(要绘制的图像数.可以设置为1以使用堆栈索引开始作为图像选择器)',
    ],
    total_frames: [
      'int',
      'decal',
    ...
    xOffsetRelative: [
      ...
      'decal',
      'X相对偏移',
      '用于修改decal的相对位置,不支持动态值,动态请使用basePosition',
    ],
    yOffsetRelative: [
      'float',
      'decal',
  ...
  template: {
    ...
    '@copyFrom_skipThisSection': [
      ...
      'template',
      '复制跳过节',
      '让[core]的copyFrom不从目标ini复制该节的内容',
    ],
    '@define ': [
      'other',
      'template',
